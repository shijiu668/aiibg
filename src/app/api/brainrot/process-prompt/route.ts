...
export async function POST(request: Request) {
  try {
    ...
    console.log("准备处理用户输入，原始提示词:", prompt);
...
Provide only the final image generation prompt in English, do not add any descriptive content`;
    ...
    try {
      ...
      console.log("提示词处理成功，返回内容:", resultText.substring(0, 100) + "...");

      // 返回处理后的提示词
      return NextResponse.json({ processedPrompt: resultText });
    } catch (textError: any) {
      console.error("提示词处理API错误:", textError);
      
      // 尝试使用备用方式
      try {
        console.log("使用备用方式处理提示词...");
    ...
    return NextResponse.json(
      { error: `提示词处理失败: ${error.message || "未知错误"}` },
      { status: 500 }
    );
  }
}